...
{reddit_content}"""
    ...
    # Enhance with ChatGPT if requested and API key is available
    if use_chatgpt:
        try:
            # Check if API key is set
            if not OPENAI_API_KEY and not os.getenv('OPENAI_API_KEY'):
                print("⚠️  OpenAI API key not found. Add your API key to use ChatGPT enhancement.")
                print("Set it as environment variable OPENAI_API_KEY or update the script.")
                final_report = basic_report
            else:
                print(f"🤖 Enhancing report for {ingredient_name} with ChatGPT...")
                final_report = enhance_report_with_chatgpt(basic_report, ingredient_name)
        except Exception as e:
            print(f"ChatGPT enhancement failed: {e}")
            final_report = basic_report
    else:
        final_report = basic_report
